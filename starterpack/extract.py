...
def shutil_copytree(src, dst, symlinks=False, ignore=None, copy_function=shutil.copy2,
    ...
    # !!!!! modified bit. There's very likely a better way to do this, but I'm too tired to figure out how.
    # (The modified bit is setting exist_ok to True, it was causing errors to happen for some reason.)
            ...
            if os.path.islink(srcname):
                ...
                if symlinks:
                    ...
                    if os.path.isdir(srcname):
                        shutil_copytree(srcname, dstname, symlinks, ignore,
                                 copy_function)
                    else:
                        copy_function(srcname, dstname)
            elif os.path.isdir(srcname):
                shutil_copytree(srcname, dstname, symlinks, ignore, copy_function)
            else:
...
def unpack_dmg(filename, dest):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            shutil_copytree(tmpdir, dest, symlinks=True, ignore=lambda _, _fs: '.Trashes')
        finally:
